...
const getUID = () => localStorage.getItem('bf_google_uid') || 'anon';
const getBase = () => `https://firestore.googleapis.com/v1/projects/${FB_PROJECT}/databases/(default)/documents/users/${getUID()}/data`;
...
const getToken = async () => {
  ...
  try {
    if (_ref) {
      const r = await fetch('https://securetoken.googleapis.com/v1/token?key='+FB_KEY, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({grant_type:'refresh_token',refresh_token:_ref})});
      const d = await r.json();
      if (d.id_token) { _tok=d.id_token; _tokExp=now+parseInt(d.expires_in)*1000; _ref=d.refresh_token; localStorage.setItem('bf_ref',_ref); return _tok; }
    }
    const r = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key='+FB_KEY, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({returnSecureToken:true})});
    const d = await r.json();
    _tok=d.idToken; _tokExp=now+parseInt(d.expiresIn)*1000; _ref=d.refreshToken; localStorage.setItem('bf_ref',_ref); return _tok;
  } catch { return null; }
};
const safeGet = async k => {
  try {
    const t = await getToken();
    const r = await fetch(`${getBase()}/${k}?key=${FB_KEY}`, {headers: t ? {Authorization:'Bearer '+t} : {}});
    if (!r.ok) return null;
    const d = await r.json();
    const v = d?.fields?.value?.stringValue;
...
const safeSet = async (k, v) => {
  try {
    ...
    await fetch(`${getBase()}/${k}?key=${FB_KEY}&updateMask.fieldPaths=value`, {
      ...
      body: JSON.stringify({fields:{value:{stringValue:JSON.stringify(v)}}})
    });
  } catch {}
};
