...
const FB_BASE = 'https://firestore.googleapis.com/v1/projects/becoming-firdaus-1/databases/(default)/documents/storage';
const safeGet = async k => { try { const r=await fetch(`${FB_BASE}/${k}?key=${FB_KEY}`); if(!r.ok) return null; const d=await r.json(); const v=d?.fields?.value?.stringValue; return v?JSON.parse(v):null; } catch { return null; } };
