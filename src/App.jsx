...
const getToken=async()=>{const now=Date.now();if(_tok&&now<_tokExp-60000)return _tok;try{const {initFirebase}=await import("./GoogleAuth.jsx");const {getAuth}=await import("firebase/auth");const auth=getAuth(initFirebase());const u=auth.currentUser;if(!u)return null;_tok=await u.getIdToken(true);_tokExp=now+3500000;localStorage.setItem("bf_google_token",_tok);return _tok;}catch{return null;}};
